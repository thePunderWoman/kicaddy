            ...
            if validate {
                ...
                match compiler.compile(&yaml_sch) {
                    Ok(compile_output) => {
                        ...
                        // Keep a pre-existing .kicad_pro's cached sheet UUID list in sync with
                        // what was just written — otherwise it's stale until KiCad's GUI
                        // silently self-corrects it on next open ("was automatically fixed").
                        if let Some(hierarchy_root_uuid) = &compile_output.root.hierarchy_root_uuid {
                            ...
                            let child_sheets: Vec<_> = compile_output
                                ...
                                .collect();
                            if let Err(e) = libkicaddy::project::sync_kicad_pro_sheets(
                                &output_path,
                                &root_sheet,
                                &child_sheets,
                            ) {
                                eprintln!("Warning: failed to sync .kicad_pro: {}", e);
                            }
                        }
